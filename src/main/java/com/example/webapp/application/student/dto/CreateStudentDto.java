...
public record CreateStudentDto(
        ...
        @NotBlank(message = "email is required") @Email(message = "email must be valid") @Size(max = 200, message = "email must be at most 200 chars") String email,
        @NotNull(message = "age is required") @Min(value = 3, message = "age must be >= 3") @Max(value = 150, message = "age must be <= 150") Short age,
        CreateStudentProfileDto profile) {

    @Override
    public final String toString() {
        return String.format(
                "CreateStudentDto [firstname=%s, lastname=%s, email=%s, age=%d]",
